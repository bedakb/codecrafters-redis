...
const crlf = "\r\n"
const sizeOfCRLF = len(crlf)

// Result holds the information about parsed RESP contents.
type Result struct {
	Type  RedisType
	Value interface{}
}

// Decode decodes RESP string into Result.
func Decode(data []byte) Result {
	v, _ := decode(data)
	return v
}

// Encode encodes given redis result into RESP string.
...
func matchCRLF(value []byte) (pos, end int) {
	i := bytes.Index(value, []byte(crlf))
	return i, i + sizeOfCRLF
}
